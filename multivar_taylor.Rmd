...
```
```{r}
fc.short = forecast(fit.mlp2, h=1440)
```

```{r}
fc.long = forecast(fit.mlp2, h=10080)
```
...
```{r}
end.idx = length(mlp.train2$Open)
start.idx = end.idx - 1440+1

mlp.short.ASE = mean((mlp.train2$Open[start.idx:end.idx] - fc.short$f)^2)
mlp.short.ASE
# ASE = 4,703,662
sqrt(mlp.short.ASE)
# Root ASE = 2,168.793
...
```{r}
end.idx = length(fc.short$f)
start.idx = end.idx - 10800+1
...
# Root ASE = 2,168.793
```
TODO: Double check this score

# Ensembling Methods

We'll expand our number of networks to train in the nnfor::mlp by using the reps parameter. This results in an ensemble forecast.

```{r}
xregs = data.frame(Count = ts(mlp.train2$Count), Volume = ts(mlp.train2$Volume), Target = ts(mlp.train2$Target))
fit.mlp.ensemble = mlp(tsdat2, xreg = xregs)
fit.mlp.ensemble
```

```{r}
plot(fit.mlp.ensemble)
```
